...
enum class SortType(@StringRes val displayTitle: Int, val comparator: Comparator<DisplayItem.Center>) {
    ByDate(R.string.sort_by_date, compareBy<DisplayItem.Center, Date?>(nullsLast()) { it.nextSlot }.thenByDescending{ it.isValidAppointmentByPhoneOnly }.thenBy(nullsLast()) { it.distance }),
